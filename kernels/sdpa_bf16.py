...
# This version only support bfloat16 datatype, used for ncu
# the original version can choose datatype with conditional setting
# but it cause problem when profiling using ncu
# when profiling using ncu, need to change the kernel used in function SDPA to this version

@triton.jit
def _att_fwd_inner_bf16(
    o_i, l_i, m_i, q_i, 
    ...
    for id_col in tl.range(low, high, Bc):
        ...
        # computation
        # the input_precision arg in tl.dot must be set at compile time
...
def att_fwd_kernel_bf16(
    ...
    if not is_causal: 
        o_i, l_i, m_i = _att_fwd_inner_bf16(
            o_i, l_i, m_i, q_i, 
            k_block_ptr_base, v_block_ptr_base, 
            qk_scale, idx_row, T,
